...
fun day20part1() {
    day20common(maxCheatDuration = 2, minSaving = 100)
}

fun day20part2() {
    day20common(maxCheatDuration = 20, minSaving = 100)
}

private fun day20common(maxCheatDuration: Int, minSaving: Int) {
    ...
    fun traceAndCheat(): MutableMap<Int, Int> {
        ...
        while (row != end.row || column != end.column) {
            ...
            for (rowOffset in -maxCheatDuration..maxCheatDuration) {
                for (columnOffset in -maxCheatDuration..maxCheatDuration) {
                    val cheatLength = abs(rowOffset) + abs(columnOffset)
                    if (cheatLength > maxCheatDuration) {
                        continue
                    }
    ...
    println(saves.filterKeys { it >= minSaving }.values.sum())
}
